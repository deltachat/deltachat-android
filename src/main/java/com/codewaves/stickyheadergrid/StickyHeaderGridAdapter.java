...
/** Created by Sergej Kravcenko on 4/24/2017. Copyright (c) 2017 Sergej Kravcenko */
@SuppressWarnings({"unused", "WeakerAccess"})
public abstract class StickyHeaderGridAdapter
    extends RecyclerView.Adapter<StickyHeaderGridAdapter.ViewHolder> {
  ...
  public int getItemSectionOffset(int section, int position) {
    ...
    if (localPosition >= sectionObject.length) {
      throw new IndexOutOfBoundsException(
          "localPosition: " + localPosition + " >=" + sectionObject.length);
    }
  ...
  /**
   * Returns the section index having item or header with provided provider <code>position</code>.
   *
   * @param position adapter position
  ...
  /**
   * Returns the adapter position for given <code>section</code> header. Use this only for {@link
   * RecyclerView#scrollToPosition(int)} or similar functions. Never directly manipulate adapter
   * items using this position.
   *
   * @param section section to query
  ...
  /**
   * Returns the adapter position for given <code>section</code> and <code>offset</code>. Use this
   * only for {@link RecyclerView#scrollToPosition(int)} or similar functions. Never directly
   * manipulate adapter items using this position.
   *
   * @param section section to query
  ...
  /**
   * Return the view type of the <code>section</code> header for the purposes of view recycling.
   *
   * <p>The default implementation of this method returns 0, making the assumption of a single view
   * type for the headers. Unlike ListView adapters, types need not be contiguous. Consider using id
   * resources to uniquely identify item view types.
   *
   * @param section section to query
   * @return integer value identifying the type of the view needed to represent the header in <code>
   *     section</code>. Type codes need not be contiguous.
   */
  ...
  /**
   * Return the view type of the item at <code>position</code> in <code>section</code> for the
   * purposes of view recycling.
   *
   * <p>The default implementation of this method returns 0, making the assumption of a single view
   * type for the adapter. Unlike ListView adapters, types need not be contiguous. Consider using id
   * resources to uniquely identify item view types.
   *
   * @param section section to query
   * @param offset section position to query
   * @return integer value identifying the type of the view needed to represent the item at <code>
   *     position</code> in <code>section</code>. Type codes need not be contiguous.
   */
  ...
  /**
   * Called when RecyclerView needs a new {@link HeaderViewHolder} of the given type to represent a
   * header.
   *
   * <p>This new HeaderViewHolder should be constructed with a new View that can represent the
   * headers of the given type. You can either create a new View manually or inflate it from an XML
   * layout file.
   *
   * <p>The new HeaderViewHolder will be used to display items of the adapter using {@link
   * #onBindHeaderViewHolder(HeaderViewHolder, int)}. Since it will be re-used to display different
   * items in the data set, it is a good idea to cache references to sub views of the View to avoid
  ...
  /**
   * Called when RecyclerView needs a new {@link ItemViewHolder} of the given type to represent an
   * item.
   *
   * <p>This new ViewHolder should be constructed with a new View that can represent the items of
   ...
   * #onBindItemViewHolder(ItemViewHolder, int, int)}. Since it will be re-used to display different
   * items in the data set, it is a good idea to cache references to sub views of the View to avoid
   * unnecessary {@link View#findViewById(int)} calls.
   *
   * @param parent The ViewGroup into which the new View will be added after it is bound to an
   ...
   * the contents of the {@link HeaderViewHolder#itemView} to reflect the header at the given
   * position.
   *
   ...
   * invalidated or the new position cannot be determined. For this reason, you should only use the
   * <code>section</code> parameter while acquiring the related header data inside this method and
   * should not keep a copy of it. If you need the position of a header later on (e.g. in a click
   * listener), use {@link HeaderViewHolder#getAdapterPosition()} which will have the updated
   * adapter position. Then you can use {@link #getAdapterPositionSection(int)} to get section
   * index.
   *
  ...
  /**
   * Called by RecyclerView to display the data at the specified position. This method should update
   * the contents of the {@link ItemViewHolder#itemView} to reflect the item at the given position.
   *
   * <p>Note that unlike {@link android.widget.ListView}, RecyclerView will not call this method
   * again if the position of the item changes in the data set unless the item itself is invalidated
   * or the new position cannot be determined. For this reason, you should only use the <code>offset
   * </code> and <code>section</code> parameters while acquiring the related data item inside this
   * method and should not keep a copy of it. If you need the position of an item later on (e.g. in
   * a click listener), use {@link ItemViewHolder#getAdapterPosition()} which will have the updated
   * adapter position. Then you can use {@link #getAdapterPositionSection(int)} and {@link
   * #getItemSectionOffset(int, int)}
   *
   ...
   *
   * <p>There are two different classes of data change events, item changes and structural changes.
   * Item changes are when a single item has its data updated but no positional changes have
   * occurred. Structural changes are when items are inserted, removed or moved within the data set.
   *
   * <p>This event does not specify what about the data set has changed, forcing any observers to
   * assume that all existing items and structure may no longer be valid. LayoutManagers will be
   * forced to fully rebind and relayout all visible views.
   *
   * <p><code>RecyclerView</code> will attempt to synthesize visible structural change events for
   * adapters that report that they have {@link #hasStableIds() stable IDs} when this method is
   * used. This can help for the purposes of animation and visual object persistence but individual
   * item views will still need to be rebound and relaid out.
   *
   * <p>If you are writing an adapter it will always be more efficient to use the more specific
   * change events if you can. Rely on <code>notifyDataSetChanged()</code> as a last resort.
   *
  ...
  public void notifySectionItemChanged(int section, int position) {
    ...
    if (mSections == null) {
      ...
      if (position >= sectionObject.itemNumber) {
        throw new IndexOutOfBoundsException(
            "Invalid index " + position + ", size is " + sectionObject.itemNumber);
      }
  ...
  public void notifySectionItemRangeInserted(int section, int position, int count) {
    ...
    if (mSections == null) {
      ...
      if (position + count > sectionObject.itemNumber) {
        throw new IndexOutOfBoundsException(
            "Invalid index " + (position + count) + ", size is " + sectionObject.itemNumber);
      }
