...
public class StickyHeaderGridLayoutManager extends RecyclerView.LayoutManager
  ...
  /** The interface to be implemented by listeners to header events from this LayoutManager. */
  public interface HeaderStateChangeListener {
     ...
     *
     * <ul>
     *   <li>NORMAL - the section header is invisible or has normal position
     *   <li>STICKY - the section header is sticky at the top of RecyclerView
     *   <li>PUSHED - the section header is sticky and pushed up by next header </ul
     *
   ...
   *
   * @param spanSizeLookup {@link StickyHeaderGridLayoutManager.SpanSizeLookup} instance to be used
   *     to query number of spans occupied by each item
   */
  ...
  /**
   * Returns the current {@link StickyHeaderGridLayoutManager.SpanSizeLookup} used by the
   * StickyHeaderGridLayoutManager.
   *
   * @return The current {@link StickyHeaderGridLayoutManager.SpanSizeLookup} used by the
   *     StickyHeaderGridLayoutManager.
   */
  ...
  /**
   * Returns the current {@link StickyHeaderGridLayoutManager.HeaderStateChangeListener} used by the
   * StickyHeaderGridLayoutManager.
   *
   * @return The current {@link StickyHeaderGridLayoutManager.HeaderStateChangeListener} used by the
   *     StickyHeaderGridLayoutManager.
   */
   ...
   *
   * @param listener {@link StickyHeaderGridLayoutManager.HeaderStateChangeListener} instance to be
   *     used to receive header state changes
   */
  ...
  /**
   * Sets the size of header bottom margin that overlaps first section item. Used to create header
   * bottom edge shadows.
   *
  ...
  public void onAdapterChanged(RecyclerView.Adapter oldAdapter, RecyclerView.Adapter newAdapter) {
    ...
    try {
      ...
      throw new ClassCastException(
          "Adapter used with StickyHeaderGridLayoutManager must be kind of StickyHeaderGridAdapter");
    }
   ...
   *
   * <p>RecyclerView will scroll the minimum amount that is necessary to make the target position
   * visible.
   *
  ...
  public void smoothScrollToPosition(
      final RecyclerView recyclerView, RecyclerView.State state, int position) {
  ...
  public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
    ...
    if (mPendingScrollPosition >= 0) {
      ...
      pendingAdapterPosition =
          getAdapterPositionChecked(
              mPendingSavedState.mAnchorSection, mPendingSavedState.mAnchorItem);
    ...
    // If layout was caused by the pending scroll, adjust top item position and move it under sticky
    // header
    if (mPendingScrollPosition >= 0) {
      mPendingScrollPosition = NO_POSITION;
    ...
    if (section >= 0 && mAdapter.isSectionHeaderSticky(section)) {
      ...
      if (offset >= 0) {
        ...
        if (mFloatingHeaderView != null && headerAdapterPosition == mFloatingHeaderPosition) {
          return Math.max(
              0, getDecoratedMeasuredHeight(mFloatingHeaderView) - mHeaderOverlapMargin);
        } else {
  ...
  private int findFirstRowItem(int adapterPosition) {
    ...
    while (sectionPosition > 0
        && mSpanSizeLookup.getSpanIndex(section, sectionPosition, mSpanCount) != 0) {
      sectionPosition--;
      adapterPosition--;
    }
  ...
  private FillResult fillBottomRow(
      RecyclerView.Recycler recycler, RecyclerView.State state, int position, int top) {
  ...
  private void clearHiddenRows(
      RecyclerView.Recycler recycler, RecyclerView.State state, boolean top) {
      ...
      while (row.bottom < recyclerTop - getExtraLayoutSpace(state) || row.top > recyclerBottom) {
        if (row.header) {
          removeAndRecycleViewAt(
              mHeadersStartPosition + (mFloatingHeaderView != null ? 1 : 0), recycler);
        } else {
  ...
  private void clearViewsAndStickHeaders(
      RecyclerView.Recycler recycler, RecyclerView.State state, boolean top) {
  ...
  private void addRow(
    ...
    if (viewType == TYPE_HEADER) {
      ...
      if (isTop) {
        ...
        mLayoutRows.add(
            0, new LayoutRow(result.adapterPosition, result.length, top - result.height, top));
      } else {
        final FillResult result = fillBottomRow(recycler, state, adapterPosition, top);
        mLayoutRows.add(
            new LayoutRow(result.adapterPosition, result.length, top, top + result.height));
      }
    }
  }

  private void addOffScreenRows(
      RecyclerView.Recycler recycler,
      RecyclerView.State state,
      int recyclerTop,
      int recyclerBottom,
    ...
    if (bottom) {
      ...
      while (true) {
        ...
        if (bottomRow.bottom >= recyclerBottom + getExtraLayoutSpace(state)
            || adapterPosition >= state.getItemCount()) {
          break;
        }
  ...
  public int scrollVerticallyBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State state) {
    ...
    if (dy >= 0) {
      ...
      while (scrolled < dy) {
        final LayoutRow bottomRow = getBottomRow();
        final int scrollChunk =
            -Math.min(Math.max(bottomRow.bottom - recyclerBottom, 0), dy - scrolled);
  ...
  private void onHeaderChanged(int section, View view, HeaderState state, int pushOffset) {
    ...
    final boolean headerStateChanged =
        mStickyHeaderSection != section
            || !mStickyHeadeState.equals(state)
            || state.equals(HeaderState.PUSHED);
  ...
  private void onHeaderUnstick() {
    if (mStickyHeaderSection != NO_POSITION) {
      if (mHeaderStateListener != null) {
        mHeaderStateListener.onHeaderStateChanged(
            mStickyHeaderSection, mStickyHeaderView, HeaderState.NORMAL, 0);
      }
  ...
  private void stickTopHeader(RecyclerView.Recycler recycler) {
    ...
    if (firstHeader != NO_POSITION) {
      ...
      if (mAdapter.isSectionHeaderSticky(section)) {
        ...
        onHeaderChanged(
            section,
            firstHeaderRow.headerView,
            offset == 0 ? HeaderState.STICKY : HeaderState.PUSHED,
            offset);
      } else {
        onHeaderUnstick();
        mStickOffset = 0;
  ...
  private void updateTopPosition() {
    ...
    if (firstVisibleRow != null) {
      ...
      mAnchor.item =
          mAdapter.getItemSectionOffset(mAnchor.section, firstVisibleRow.adapterPosition);
   ...
   *
   * <p>Default implementation sets each item to occupy exactly 1 span.
   *
   * @see
   *     StickyHeaderGridLayoutManager#setSpanSizeLookup(StickyHeaderGridLayoutManager.SpanSizeLookup)
   */
  public abstract static class SpanSizeLookup {
    /**
     * Returns the number of span occupied by the item in <code>section</code> at <code>position
     * </code>.
     *
     ...
     *
     * <p>If you override this method, you need to make sure it is consistent with {@link
     * #getSpanSize(int, int)}. StickyHeaderGridLayoutManager does not call this method for each
     * item. It is called only for the reference item and rest of the items are assigned to spans
     * based on the reference item. For example, you cannot assign a position to span 2 while span 1
     * is empty.
     *
     * <p>
     *
     ...
     * @return The final span position of the item. Should be between 0 (inclusive) and <code>
     *     spanCount</code>(exclusive)
     */
    public int getSpanIndex(int section, int position, int spanCount) {
      // TODO: cache them?
