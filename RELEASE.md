...
2. update weblate translations of metadata:
   ```
   git remote add weblate https://translate.codeberg.org/git/delta-chat/android-metadata/
   git fetch weblate
   git merge --no-ff weblate/main
   ```
...
5. add a device message to `ConversationListActivity::onCreate()` or remove the old one.
...
6. bump `versionCode` _and_ `versionName` (no leading "v") in `build.gradle`
...
8. a) `./scripts/upload-beta.sh VERSION` uploads both APKs to testrun.org and drafts a message.  
...
9. a) `./scripts/upload-release.sh VERSION`  
...
10. a) open "Delta Chat/Release/Production"
...
11. Add "4" at the end of versionCode to calculate F-Droid version code number.
...
12. make sure, everything is pushed, then:  
...
13. a) for "Delta Chat", select "Add upcoming version" on the left
